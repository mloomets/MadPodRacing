...
public:
    ...
    Goal(int x, int y, Goal *next) :
            _x(x), _y(y), _distNext(distanceTo(next->_x, next->_y)), _next(next), _prev(nullptr)
    {}
...
public:
    ...
    [[nodiscard]] Vector rotatedValue(double angle) const
    {
        ...
        return {_x * cosVal - _y * sinVal, _x * cosVal + _y * sinVal};
    }
...
public:
    ...
    std::string getAction()
    {
        ...
        double distanceMultiplier = std::clamp(pow(nextGoalDist, 2) / pow(distMultRange, 2), 0.5, 1.0);
        double nextAngleMultiplier =
                (Vector(goals[nextGoalId]) - location).len() < 3000 && strategy != Strategy::ATTACK ?
                std::clamp(getNextAngleMultiplier(), 0.25, 1.0)
                                                                                                    : 1.0;
        ...
        double angleMultiplier = std::abs(angle) < angleMultThreshold ?
                                 std::clamp((1 - (nextGoalAngle / maxAbsAngle)), 0.1, 1.0)
                                                                      : 1.0;
#ifdef LOG
        ...
        int thrust = static_cast<int>(maxThrust * distanceMultiplier * nextAngleMultiplier * enemyDistanceMultiplier *
                                      angleMultiplier);
        if (strategy == Strategy::ATTACK && aboutToCollideFriendly())
        { thrust = 0; }
...
private:
    ...
    Vector getTargetLocation()
    {
        switch (strategy)
        {
            case Pod::Strategy::RACE:return getNextPointCorrectedTarget(checkpointSize * 0.8);
            case Pod::Strategy::ATTACK:return getBestEnemyTarget();
        }
    }
    ...
    {
        return std::min((enemyPods->first.location - location).len(), (enemyPods->second.location - location).len());
    }

    [[nodiscard]] bool aboutToCollideEnemy() const
    {
        Vector nextLocation = location + speed;
        return std::min((nextLocation - enemyPods->first.location - enemyPods->first.speed).len(), (nextLocation -
                                                                                                    enemyPods->second.location -
                                                                                                    enemyPods->second.speed).len()) <
               850;
    ...
    {
        return (pods->second.location + pods->second.calcSpeed - pods->first.location - pods->first.calcSpeed).len() <
               1000;
    }
...
void calculateNextPointVectors()
{
    for (int i = 0; i < goals.size(); i++)
    {
        ...
        Vector toNext(goals[i].getX(), goals[i].getY(), goals[nextIndex].getX(), goals[nextIndex].getY());
        toNext.normalize();
